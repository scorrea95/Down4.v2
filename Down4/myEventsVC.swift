...
class myEventsVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func fetchEvents() {
        ...
        Database.database().reference().child("users/\(Auth.auth().currentUser!.uid)/eventPost").observe(.childAdded, with: { (snapshot) in
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if events.count > 0 {
            ...
            collectionVIew.backgroundView = UIImageView(image: #imageLiteral(resourceName: "noeventscreated"))
