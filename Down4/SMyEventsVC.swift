...
class SMyEventsVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func fetchEvents() {
        ...
        Database.database().reference().child("users/\(userDetails!.uid!)/eventPost").observe(.childAdded, with: { (snapshot) in
